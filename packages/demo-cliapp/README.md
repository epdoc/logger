...
This package provides three demo implementations showing how to use **@epdoc/cliffapp** to build structured, type-safe
CLI applications with built-in logging and context management.

## CLI Patterns

We support three primary patterns for building your CLI. You can choose the one that best fits your complexity or mix
them together.
...
- **Best for**: Large CLIs where the root/core is structured, but many leaf commands are simple enough to be defined
  inline.
- **Key Features**:
...
One of the unique features of **cliffapp** is the ability to refine the application context at every level of the
command hierarchy.
...
3. **Dynamic Subcommands**: If `subCommands` is a function, it is called _after_ refinement, allowing you to dynamically
   show or hide subcommands based on earlier flags (e.g., hiding admin commands unless `--admin` is passed).
