...
- Finished error free version of progress
...
- Fix MCP stdout flushing and add error handling Use writeSync in writeMessage to ensure MCP JSON-RPC responses are
  immediately flushed to stdout, preventing inspector timeouts. Add try/catch error handling in serve loop to prevent
  server crashes on request processing errors. Fixes MCP inspector disconnect issues when running as stdio transport.
...
## [2.0.6-alpha.1] - 2026-02-08

- Tweaking context types
