...
Deno.test('MsgBuilder.Console', async (t) => {
  ...
  await t.step('specific methods', async (t2) => {
    await t2.step('h1', () => {
      ...
      assert.deepStrictEqual(result, enable.h1 + 'h1' + disable.h1);
    });

    await t2.step('h2', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.h2('h2').format({ color: true });
      assert.deepStrictEqual(result, enable.h2 + 'h2' + disable.h2);
    });

    await t2.step('h3', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.h3('h3').format({ color: true });
      assert.deepStrictEqual(result, enable.h3 + 'h3' + disable.h3);
    });

    await t2.step('action', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.action('action').format({ color: true });
      assert.deepStrictEqual(result, enable.action + 'action' + disable.action);
    });

    await t2.step('label', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.label('label').format({ color: true });
      assert.deepStrictEqual(result, enable.label + 'label' + disable.label);
    });

    await t2.step('highlight', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.highlight('highlight').format({ color: true });
      assert.deepStrictEqual(result, enable.highlight + 'highlight' + disable.highlight);
    });

    await t2.step('value', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.value('value').format({ color: true });
      assert.deepStrictEqual(result, enable.value + 'value' + disable.value);
    });
    ...
    await t2.step('path', () => {
      ...
      console.log(result);
      assert.deepStrictEqual(result, enable.path + 'path' + disable.path);
    });
    ...
    await t2.step('relative to home', () => {
      ...
      assert.deepStrictEqual(result, enable.path + '~/relative/to/home' + disable.path);
      const result2 = msgBuilder.relative(path).format({ color: true });
      assert.deepStrictEqual(result2, enable.path + '~/relative/to/home' + disable.path);
    });

    await t2.step('date with string (fallback)', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.date('date').format({ color: true });
      assert.deepStrictEqual(result, enable.date + 'date' + disable.date);
    });
    ...
    await t2.step('date with Temporal.ZonedDateTime', () => {
      ...
      assert.deepStrictEqual(result, '10:30:45');
    });
    ...
    await t2.step('warn', () => {
      ...
      assert.deepStrictEqual(result, enable.warn + 'warn' + disable.warn);
    });

    await t2.step('error', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.error('error').format({ color: true });
      assert.deepStrictEqual(result, enable.error + 'error' + disable.error);
    });

    await t2.step('strikethru', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.strikethru('strikethru').format({ color: true });
      assert.deepStrictEqual(result, enable.strikethru + 'strikethru' + disable.strikethru);
    });

    await t2.step('url', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.url('https://example.com').format({ color: true });
      assert.deepStrictEqual(result, enable.url + 'https://example.com' + disable.url);
    });

    await t2.step('code', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.code('const foo = 42;').format({ color: true });
      assert.deepStrictEqual(result, enable.code + 'const foo = 42;' + disable.code);
    });

    await t2.step('success', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.success('Operation completed').format({ color: true });
      assert.deepStrictEqual(result, enable.success + 'Operation completed' + disable.success);
    });

    await t2.step('dim', () => {
      const msgBuilder = new MsgBuilder.Console.Builder();
      const result = msgBuilder.dim('secondary info').format({ color: true });
      assert.deepStrictEqual(result, enable.dim + 'secondary info' + disable.dim);
    });
  });
  ...
  await t.step('icon methods', async (t2) => {
    ...
    await t2.step('bool method', async (t3) => {
      await t3.step('bool(true) with default preset (check)', () => {
        ...
        assert.deepStrictEqual(result, '\x1b[38;2;81;214;124m✓\x1b[39m');
      });

      await t3.step('bool(false) with default preset (check)', () => {
        const msgBuilder = new MsgBuilder.Console.Builder();
        const result = msgBuilder.bool(false).format({ color: true });
        assert.deepStrictEqual(result, '\x1b[38;2;239;68;68m✗\x1b[39m');
      });
      ...
      await t3.step('bool(true) with custom trueColor', () => {
        ...
        const result = msgBuilder
          ...
          .format({ color: true });
        assert.deepStrictEqual(result, '\x1b[38;2;0;255;0m✓\x1b[39m');
      });
      ...
      await t3.step('bool(false) with custom falseColor', () => {
        ...
        const result = msgBuilder
          ...
          .format({ color: true });
        assert.deepStrictEqual(result, '\x1b[38;2;255;0;0m✗\x1b[39m');
      });
  ...
  await t.step('count method for pluralization', async (t2) => {
    ...
    await t2.step('singular with two arguments: uses first string', () => {
      ...
      assert.deepStrictEqual(result, '1 entry');
    });
    ...
    await t2.step('plural with two arguments: uses second string', () => {
      ...
      assert.deepStrictEqual(result, '5 entries');
    });
  ...
  await t.step('standalone usage', async (t2) => {
    ...
    await t2.step('emit() outputs with color', () => {
      ...
      if (result) {
        ...
        assert.ok(result.timestamp instanceof DateTime);
        assert.ok(result.formatter instanceof MsgBuilder.Console.Builder);
        assert.deepStrictEqual(tester.output, enable.h2 + 'test' + disable.h2);
      }
    });
    ...
    await t2.step('emit() outputs with nocolor', () => {
      ...
      if (result) {
        ...
        assert.ok(result.timestamp instanceof DateTime);
        assert.ok(result.formatter instanceof MsgBuilder.Console.Builder);
        assert.deepStrictEqual(tester.output, 'test');
      }
    });
  });
  ...
  await t.step('style map configuration', async (t2) => {
    ...
    await t2.step('subclass can have its own style theme', () => {
      ...
      assert.deepStrictEqual(MsgBuilder.Console.Builder.styleFormatters, originalStyles);
    });
  });
});
